...
"""Main stdio MCP server for the Phala reference implementation.

MCP scaffolding portable across sibling protocol repos (acap,
pratyahara-nerve, sauvidya-pace). When copying this file, change only:

  - SERVER_NAME, SERVER_VERSION, SERVER_INSTRUCTIONS
  - the `from phala.mcp_server.tools import ...` line
...
from phala.mcp_server.tools import HANDLERS, TOOL_SCHEMAS, ToolInvocationError
...
def main() -> None:
    """Entry point for ``python -m phala.mcp_server`` and the ``phala-mcp`` script.
